      ...
      <ShopLayout showFooter={true} showWhatsApp={false}>
        <main className="min-h-screen bg-white pt-8">
          <div className="mx-auto max-w-[1400px] px-6 md:px-12">
            ...
            <div className="mb-8 grid grid-cols-1 gap-6 md:mb-20 md:gap-12 lg:grid-cols-2">
              <ImageGallery
                images={images}
...
function Breadcrumb({ product }: { product: ApiProduct }) {
  return (
    <nav className="mb-4 flex items-center gap-2 text-xs text-neutral-500 md:mb-8 md:text-sm">
      ...
      <Link
        href="/shop/products"
        className="hidden hover:text-neutral-900 sm:inline"
      >
        Produk
      </Link>
      ...
      {product.category && (
        <>
          <Link
            ...
            className="max-w-[100px] truncate hover:text-neutral-900 sm:max-w-none"
          >
      ...
      <span className="max-w-[150px] truncate text-neutral-900 sm:max-w-[200px]">
        {product.name}
      </span>
    </nav>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2 md:top-6 md:left-6">
          {product.has_discount && (
            <span className="w-fit rounded-sm bg-red-500 px-2 py-1 text-xs font-medium text-white md:px-4 md:py-2">
              -{product.discount_percentage}%
            </span>
          )}
          {product.sale_type?.value &&
            product.sale_type.value !== 'regular' && (
              <span className="rounded-sm bg-teal-600 px-2 py-1 text-xs font-medium text-white md:px-4 md:py-2">
                {product.sale_type.label}
              </span>
            )}
        </div>
        <button className="absolute right-3 bottom-3 rounded-sm bg-white/90 p-2 text-neutral-900 hover:bg-white md:right-6 md:bottom-6 md:p-3">
          <ZoomIn size={18} className="md:h-5 md:w-5" />
        </button>
        {images.length > 1 && (
          <>
            <button
              ...
              className="absolute top-1/2 left-2 -translate-y-1/2 rounded-sm bg-white/90 p-2 text-neutral-900 hover:bg-white md:left-4 md:p-3"
            >
            ...
            <button
              ...
              className="absolute top-1/2 right-2 -translate-y-1/2 rounded-sm bg-white/90 p-2 text-neutral-900 hover:bg-white md:right-4 md:p-3"
            >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <span className="text-neutral-300">|</span>
        <span className="text-neutral-500">{product.sold_count} terjual</span>
      </div>
      <div className="border-y border-neutral-100 py-4">
        {product.has_discount ? (
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            ...
            <span className="text-lg text-neutral-400 line-through md:text-xl">
              {product.price_formatted}
            </span>
            <span className="rounded-sm bg-red-600 px-2 py-1 text-xs font-bold text-white md:text-sm">
              -{product.discount_percentage}%
            </span>
          </div>
        ) : (
      ...
      <div className="flex items-center gap-2">
        {product.is_in_stock ? (
          <>
            ...
            <span className="font-medium text-green-600">Stok Tersedia</span>
            {product.track_stock && (
              <span className="text-neutral-500">
      ...
      <div className="fixed right-0 bottom-0 left-0 z-40 mb-0 flex items-center gap-3 bg-white p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] md:static md:mb-6 md:bg-transparent md:p-0 md:shadow-none">
        <div className="flex h-[48px] items-center rounded-sm border border-neutral-200 text-neutral-900 md:h-[56px]">
          <button
            onClick={() => setQuantity(Math.max(1, quantity - 1))}
            className="flex h-full w-10 items-center justify-center border-r border-neutral-200 transition-colors hover:bg-neutral-50 md:w-14"
            disabled={isAddingToCart}
          >
            <Minus size={16} className="md:w-[18px]" />
          </button>
          <span className="w-10 text-center text-sm font-medium md:w-14 md:text-base">
            {quantity}
          </span>
          <button
            onClick={() => setQuantity(quantity + 1)}
            disabled={
              (product.track_stock && quantity >= product.stock_quantity) ||
              isAddingToCart
            }
            className="flex h-full w-10 items-center justify-center border-l border-neutral-200 transition-colors hover:bg-neutral-50 disabled:opacity-50 md:w-14"
          >
        ...
        <button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-sm bg-teal-500 py-3 text-sm font-medium text-white transition-all hover:bg-teal-600 disabled:cursor-not-allowed disabled:opacity-50 md:gap-3 md:py-4 md:text-base"
        >
          {isAddingToCart ? (
            <>
        ...
        <button
          ...
          className={`flex items-center justify-center rounded-sm border p-3 transition-colors disabled:cursor-not-allowed disabled:opacity-50 md:p-4 ${isWishlisted ? 'border-red-200 bg-red-50 text-red-500' : 'border-neutral-200 hover:border-neutral-900'}`}
        >
          {isTogglingWishlist ? (
        ...
        <button
          onClick={onShare}
          className="hidden rounded-sm border border-neutral-200 p-4 transition-colors hover:border-neutral-900 hover:bg-neutral-50 md:flex"
        >
          <Share2 size={20} />
        </button>
      </div>
      <div className="grid grid-cols-3 gap-4 border-t border-neutral-100 py-6">
        <div className="text-center">
          ...
          <p className="text-sm font-medium text-neutral-900">Gratis Ongkir</p>
        ...
        <div className="text-center">
          <RotateCcw className="mx-auto mb-2 text-teal-600" size={24} />
          <p className="text-sm font-medium text-neutral-900">14 Hari Retur</p>
      ...
      {product.specifications &&
        Object.keys(product.specifications).length > 0 && (
          <div className="border-t border-neutral-100 pt-6">
            <h3 className="mb-4 font-medium text-neutral-900">Spesifikasi</h3>
            <dl className="grid grid-cols-2 gap-3 text-sm">
              {Object.entries(product.specifications).map(([key, value]) => (
                <div key={key} className="flex">
                  <dt className="w-24 flex-shrink-0 text-neutral-500">{key}</dt>
...
function RelatedProducts({ products }: { products: ApiProduct[] }) {
  return (
    <section className="mb-8 md:mb-20">
      <h2 className="mb-4 font-serif text-xl text-neutral-900 md:mb-8 md:text-2xl">
        Produk Terkait
      </h2>
...
function CustomerReviews({
  ...
  return (
    <section className="mb-8 border-t border-neutral-100 pt-8 md:mb-20 md:pt-12">
      <h2 className="mb-4 font-serif text-xl text-neutral-900 md:mb-8 md:text-2xl">
        Ulasan Pelanggan
      </h2>
          ...
          <div className="space-y-2">
            {ratingCounts.map(({ rating, count, percentage }) => (
              <div key={rating} className="flex items-center gap-2 text-sm">
                ...
                <span className="w-8 text-neutral-500">{count}</span>
              </div>
            ))}
          </div>
      ...
      router.put(
        ...
        { product_id: productId, rating, comment },
        {
          ...
          onError: (errors) => {
            setMessage({
              type: 'error',
              text: Object.values(errors)[0] || 'Gagal memperbarui ulasan.',
            });
          },
      ...
      router.post(
        ...
        { product_id: productId, rating, comment },
        {
          ...
          onError: (errors) => {
            setMessage({
              type: 'error',
              text: Object.values(errors)[0] || 'Gagal mengirim ulasan.',
            });
          },
          onFinish: () => setIsSubmitting(false),
        },
